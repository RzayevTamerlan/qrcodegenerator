...
const QRSelect = () => {
  ...
  return (
    <div className='flex bg-gray-800 px-10 py-7 rounded-[40px] justify-center items-center gap-3 mb-10'>
      <ul className='w-full flex items-center gap-5'>
        {qrTypesList.map((type) => (
          <li onClick={() => handleQRType(type.value)}
              className={`cursor-pointer rounded-[30px] w-fit px-5 py-4 border border-1 transition-all duration-300 ${qrType === type.value ? 'border-white' : 'border-gray-600'} border-gray-600 hover:border-white`}
              key={type.id}>
